...
function createTaskDOMInboxLoop(i, j) {
  ...
  star.addEventListener("click", () => {
    const index = projectArray[i].taskArray.findIndex(
...
function revealDetailsBoxInboxLoop(
  i,
  variable,
  box,
  title,
...
function populateDetailsBoxInboxLoop(i, task_box) {
  ...
  if (index !== -1) {
    document.querySelector(
      "#details_title"
    ).value = `${projectArray[i].taskArray[index].title}`;
    document.querySelector(
      "#details_description"
    ).value = `${projectArray[i].taskArray[index].description}`;
    document.querySelector(
      "#details_due_date"
    ).value = `${projectArray[i].taskArray[index].dueDate}`;
  }
}
...
function createDetailsDOMInboxLoop(
  i,
  afterChildDiv,
  title,
  date_btn,
...
function submitDetailsBoxInboxLoop(
  i,
  task_box,
  details_box,
  title,
  ...
  details_btn.addEventListener("click", () => {
    ...
    if (index !== -1) {
      projectArray[i].taskArray[index].title =
        document.querySelector("#details_title").value;
      projectArray[i].taskArray[index].description = document.querySelector(
        "#details_description"
      ).value;
      projectArray[i].taskArray[index].dueDate =
